...
class ProfileService:
    ...
    def ensure_profiles_folder(self) -> Path:
        ...
        try:
            ...
            raise RuntimeError(
                f"Unable to access the profiles folder:\n{error}"
            ) from error

        return self.profiles_folder

    def list_profiles(self) -> list[str]:
        """Return user-defined profile names in alphabetical order."""

        profiles_folder = self.ensure_profiles_folder()

        profile_names = {
            profile_path.stem
            for profile_path in profiles_folder.glob(
    ...
    def create_profile(
        ...
        if profile_path.exists():
            raise FileExistsError(
                f'A profile named "{normalized_name}" already exists.'
            )
    ...
    def save_profile(
        ...
        """Update an existing narration profile."""
        ...
        if not profile_path.is_file():
            raise FileNotFoundError(
                f'Profile not found: "{normalized_name}"'
            )
    ...
    def load_profile(
        ...
        """Load and validate a narration profile."""
        ...
        if not isinstance(file_data, dict):
            raise ValueError(
                "The selected file is not a valid narration profile."
            )
        ...
        if file_data.get("format_version") != self.FORMAT_VERSION:
            raise ValueError(
                "This profile uses an unsupported format version."
            )
        ...
        if not isinstance(profile_data, dict):
            raise ValueError(
                "The narration profile data is missing or invalid."
            )
    ...
    def rename_profile(
        ...
        """Rename an existing narration profile."""

        normalized_current_name = self.validate_profile_name(
            current_name
        )
        normalized_new_name = self.validate_profile_name(new_name)

        current_path = self.get_profile_path(
        ...
        if current_path == new_path:
            return self._write_profile(
                profile_name=normalized_new_name,
                profile_data=profile_data,
        ...
        try:
            ...
            raise RuntimeError(
                f"Unable to complete the profile rename:\n{error}"
            ) from error
    ...
    def delete_profile(
        ...
        """Delete an existing narration profile."""
        ...
        try:
            ...
            raise RuntimeError(
                f"Unable to delete the profile:\n{error}"
            ) from error
    ...
    def validate_profile_name(
        ...
        """Validate a user-defined profile name."""

        if not isinstance(profile_name, str):
            raise TypeError(
                "The profile name must be text."
            )
        ...
        if normalized_name.endswith((".", " ")):
            raise ValueError(
                "A profile name cannot end with a period or space."
            )
        ...
        if reserved_base_name in cls.RESERVED_WINDOWS_NAMES:
            raise ValueError(
                f'"{normalized_name}" is reserved by Windows.'
            )
    ...
    def validate_profile_data(
        ...
        """Validate and normalize narration profile settings."""

        if not isinstance(profile_data, dict):
            raise TypeError(
                "Profile data must be provided as a dictionary."
            )

        missing_fields = (
            cls.REQUIRED_FIELDS - profile_data.keys()
        )
    ...
    def _write_profile(
        ...
        """Write validated profile data atomically."""

        validated_data = self.validate_profile_data(
            profile_data
        )
        ...
        try:
            ...
            raise RuntimeError(
                f"Unable to save the profile:\n{error}"
            ) from error

        if not profile_path.is_file():
            raise RuntimeError(
                "The narration profile file was not created."
            )
    ...
    def _require_string(
        ...
        """Return a text profile value."""

        value = profile_data.get(field_name)

        if not isinstance(value, str):
            raise ValueError(
                f"Profile field '{field_name}' must be text."
            )
    ...
    def _require_integer(
        ...
        """Return an integer profile value within its allowed range."""

        value = profile_data.get(field_name)

        if isinstance(value, bool) or not isinstance(value, int):
            raise ValueError(
                f"Profile field '{field_name}' must be an integer."
            )

        if not minimum <= value <= maximum:
            raise ValueError(
                (
                    f"Profile field '{field_name}' must be between "
